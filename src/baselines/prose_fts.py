"""
PROSE-FTS (Fetch-Then-Score) — Core Ordering Ablation Baseline.

This is the SINGLE MOST IMPORTANT ablation in the paper.  It keeps ALL of
PROSE's machinery (ODUS-X ranker, PHT/PTB, MQR-ULF, P-Buffer, burst expansion,
sticky TTL) but REVERSES the ordering:

  PROSE (SBFI):    Summaries from CXL → Score → Fetch only validated
  PROSE-FTS:       Fetch full chunks from CXL → Score locally → Keep top-K

The ONLY difference is the ordering of fetch vs. score.  This isolates the
value of Score-Before-Fetch-Initiative (SBFI) from the quality of the
ranker/predictor.

Key metrics:
  - Invalid payload traffic: chunks DMA'd from CXL but discarded by local scorer
  - CXL queue utilization ρ: higher for FTS due to bulk fetches
  - Recovery@K: same ranker quality, but wasted bandwidth reduces effective budget

Expected result (HPCA claim):
  - At 90% offload ratio, PROSE-FTS wastes >40% of CXL bandwidth on invalid chunks
  - PROSE wastes 0% (all fetched chunks are used)
  - Recovery gap widens with context length (more candidates → more waste)
"""
...
class PROSEFTSPolicy(BaselinePolicy):
    """PROSE with Fetch-Then-Score ordering.

    Same predictor/ranker/buffer as PROSE, but:
      - Step 1: DMA full chunks from CXL for ALL candidates
      - Step 2: Score chunks locally using ODUS-X
      - Step 3: Keep top-K (rest are invalid traffic)
    """
    ...
    def __init__(
        ...
        # State mirrors PROSE but applies AFTER fetch
    ...
    def select_active_chunks(
        ...
        """Fetch-then-score: DMA full chunks, then score locally."""
        ...
        # ── CANDIDATE GENERATION (same as PROSE MQR-ULF) ──
        candidates = self._generate_candidates(num_chunks, attn_arr, anchor_ids)

        # ── FETCH-THEN-SCORE: DMA ALL candidates as full chunks ──
        # This is the KEY difference from PROSE
        selected, payload_result = self.cxl_session.fetch_then_score(
    ...
    # ── Internal: Candidate Generation (same logic as PROSE ULF) ──

    def _generate_candidates(
        self, num_chunks: int, attn_arr: np.ndarray, anchor_ids: List[int]
    ) -> List[int]:
        """Generate candidate set mirroring MQR-ULF's 4 queues."""
        anchor_set = set(anchor_ids)
        candidates = set()
    ...
    # ── Internal: Scoring (same as PROSE ODUS-X) ──

    def _score_chunks(
        self, candidate_ids: List[int], attn_arr: np.ndarray, anchor_ids: List[int]
    ) -> List[int]:
        """ODUS-X equivalent: multi-cue scoring.

        In PROSE this runs on 64B summaries.  In PROSE-FTS it runs on
        full chunks (same logic, but too late — data already transferred).
        """
        scores = {}
        anchor_set = set(anchor_ids)
